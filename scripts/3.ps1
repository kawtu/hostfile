...
# ── Rewrite https:// -> http:// for all mirrored domains in deployed HTML ─────
# This is the real HSTS fix — if the href still says https://uppbpb.gov.in/file.pdf
# the browser goes to the real site and hits HSTS. Rewriting to http:// means it
# hits our local mirror instead, which serves over plain HTTP with no SSL check.
Write-Host "[3.ps1:REWRITE] rewriting https links to http for mirrored domains..." -ForegroundColor Cyan
$rewriteTargets = @(
    "$htdocsPath\$baseDomain"
)
if (Test-Path $externalRoot) {
    Get-ChildItem -Path $externalRoot -Directory | ForEach-Object {
        $rewriteTargets += $_.FullName
    }
}
$rewriteCount = 0
foreach ($dir in $rewriteTargets) {
    $htmlFiles = Get-ChildItem -Path $dir -Recurse -Include "*.html","*.htm" -File -ErrorAction SilentlyContinue
    foreach ($file in $htmlFiles) {
        $content = Get-Content -Path $file.FullName -Raw -ErrorAction SilentlyContinue
        if (!$content) { continue }
        $original = $content
