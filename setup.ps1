...
$baseDomain = ($parsedUri.Host -replace '^www\.', '')
$subPath    = $parsedUri.AbsolutePath.Trim('/')
...
if (-not $XamppInstallDir) {
    ...
    try {
        ...
        while (-not (Test-Path "$XamppInstallDir\apache\bin\httpd.exe") -and (Get-Date) -lt $deadline) {
